...
http://{{GITLAB_URL_PREFIX}}.{{ROUTER_ADDRESS}}/{{GITLAB_USER}}/publicweatherdata/tree/{{PUBLICWEATHER_VERSION}}[http://{{GITLAB_URL_PREFIX}}.{{ROUTER_ADDRESS}}/{{GITLAB_USER}}/publicweatherdata/tree/{{PUBLICWEATHER_VERSION}}]
...
After you click `publicweatherdata`, on the next screen you will need to enter some configuration for the template. Leave the actual value, or to use your local clone of the Git repository, enter:
