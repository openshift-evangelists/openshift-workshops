...
[source]
----
docker.io/openshiftdemos/parksmap:0.2
----
...
does not run Docker containers as the specified user by default. In fact, when
OpenShift launches a container its user is actually randomized. While this image
specifies the `root` user in its Dockerfile, running as an arbitrary, randomized
user ID will not break the application.
