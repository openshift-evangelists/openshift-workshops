...
In this lab, we're going to deploy a backend application, developed in Python language that will expose 2 main endpoints to the main visualizer application (frontend). The application will query for national parks information (including it's coordinates) that is stored in a mongo database.
