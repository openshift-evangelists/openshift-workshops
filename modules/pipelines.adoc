...
The database configuration was created through the use of `ConfigMaps` in the previous
...
created in previous labs. Click on *Builds* &rarr; *Image* and then `nationalparks` to
...
to specify the Spring Boot application name:

* `SPRING_APPLICAITON_NAME`: `nationalparks-live`

image::/images/pipeline-live-deploy.png[National Parks Live Deploy]
