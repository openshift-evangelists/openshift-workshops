...
TODO: needs update. should oauth stuff work? Is this available in 3.3.1.3?
...
. If approved, deployed to the *Live* container
...
To create your pipeline build configuration, click *Add to Project* at the top
of the page, and then type "pipe" in the search box:

image::/images/pipeline-template.png[Pipeline Template]

Click on "dev-live-pipeline" and you will be brought to the configuration
options page. The only thing you need to do is fill out the *Project name* box,
and be sure to enter your project name, which is `explore{{USER_SUFFIX}}`:

image::/images/pipeline-template-config.png[Pipeline Template Configuration]

Then go ahead and click *Create* at the bottom of the page, and then *Continue
to Overview*. You will see the notificiation at the top of the page that you
have created *dev-pipeline-live*.

The overview page will not show anything has changed, but we'll look at what we
created in just a moment. First, let's go ahead and set up the *Live* resources:
a *Deployment Configuration* and a *Service* for both the `nationalparks`
application as well as MongoDB. This is as easy as:
...
You'll notice that new *Live* resources show up in the overview, but no pod
comes up for `nationalparks-live`. This is because the *Deployment
Configuration* for it is looking for a particular image tag, which does not yet
exist.
...
When the pipeline reaches the *Deploy Dev* stage, a new deployment takes place
to the existing `nationalparks` while leaving the *Live* container intact, not
to disrupt the live/production environment.

NOTE: You will notice on the first execution of the pipeline that there are no
containers in the *Live* environment. That is because no images have been tagged
as "live" yet, which the *Live* deployment configuration is looking for.
...
container. Visit the existing `nationalparks` web service to query for data and
...
application. The last thing you'll need to do is visit the `/data/load` endpoint
for the *Live* version of `nationalparks`:
...
Now, if you visit the parks map again, you should see the map points!
