...
discovered endpoints to visualize their map data. Check what services are labeled
with `type=parksmap-backend` in your project:

[source]
----
oc get svc -l type=parksmap-backend

NAME                 CLUSTER-IP       EXTERNAL-IP   PORT(S)    AGE
nationalparks        172.30.194.180   <none>        8080/TCP   2h
nationalparks-live   172.30.215.212   <none>        8080/TCP   2h
----

Remove the `type` label from the *Dev* service so that the `parksmap` web app
uses only the *Live* service for the map data.
